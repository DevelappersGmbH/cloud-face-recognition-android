...
import de.develappers.facerecognition.R.styleable
import de.develappers.facerecognition.utils.Bezier
import de.develappers.facerecognition.utils.ControlTimedPoints
import de.develappers.facerecognition.utils.TimedPoint
...
class SignatureView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    fun getSmallSignatureBitmap(): Bitmap {
        val originalBitmap = getTransparentSignatureBitmap()
        val smallBgBitmap = Bitmap.createBitmap(
            originalBitmap!!.width/5, originalBitmap.height/5, ARGB_8888
        )
        val canvas = Canvas(smallBgBitmap)
        canvas.drawColor(Color.WHITE)
        canvas.drawBitmap(originalBitmap, 0f, 0f, null)
        return smallBgBitmap
    }
